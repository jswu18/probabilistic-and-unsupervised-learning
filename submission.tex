...
$$P(\textbf{x}^{(n)|\textbf{p}^{(1)}}=[0.5, 0.5,...,0.5]^T, M_1) = \prod_{d=1}^{D} (0.5)^{x_d^{(n)}} (0.5)^{1-x_d^{(n)}}$$
...
$$P(\textbf{x}^{(n)}|\textbf{p}^{(2)}}=[p_d, p_d, ..., p_d]^T, M_2) = \prod_{d'=1}^{D} p_d^{x_{d'}^{(n)}} (1-p_d)^{1-x_{d'}^{(n)}}$$
...
$$P(\textbf{x}^{(n)}|\textbf{p}^{(3)}}=[p_1, p_2, ..., p_D]^T, M_3) = \prod_{d=1}^{D} p_d^{x_d^{(n)}} (1-p_d)^{1-x_d^{(n)}}$$
...
which can be vectorised as $\log \textbf{r}_{n}$ using Python scipy's $logsumexp$ operation.
...
Combining:
$$ E = \sum_n \textbf{r}_{n}^T[\log(\pi)+\log(\mathbf{P})^T x^{(n)} + \log(1-\mathbf{P})^T(1-x^{(n)})]$$
...
Similar to Question 1, we can see that taking the derivative, the term in the brackets will always be less than zero and with $r_{nk}\geq 0$ and $\sum_{n}r_{nk}=1$, the second derivative will always be negative. This ensures that we have a maximum in the next step.

Setting the derivative to zero:

$$\frac{\sum_n  x^{(n)}_{d}r_{nk}}{ \mathbf{P}_{dk}}-\frac{\sum_n r_{nk}-\sum_n  x^{(n)}_{d}r_{nk}}{1-\mathbf{P}_{dk}} = 0$$
...
$$\arg \max_{\pi, \mathbf{P}} \Bigg\langle \sum_n \log P(x^{(n)}, s^{(n)}| \pi, \mathbf{P})\Bigg\rangle_{q(\{s^{(n)}\})}$$
...
%\begin{minipage}{0.5\textwidth}
%  \centering
%  \includegraphics[scale=0.5]{outputs/q3/q3e-0-optimised-p}
%  \caption{EM optimised parameters: Trial 0}
%  \label{fig:3e-initialised-p-trial-0}
%\end{minipage}%
%\begin{minipage}{0.5\textwidth}
%  \centering
%  \includegraphics[scale=0.5]{outputs/q3/q3e-1-optimised-p}
%  \caption{EM optimised parameters: Trial 1}
%  \label{fig:3e-initialised-p-trial-1}
%\end{minipage}
%\end{figure}
...
%\begin{minipage}{0.5\textwidth}
%  \centering
%  \includegraphics[scale=0.5]{outputs/q3/q3e-2-optimised-p}
%  \caption{EM optimised parameters: Trial 2}
%  \label{fig:3e-initialised-p-trial-2}
%\end{minipage}%
%\begin{minipage}{0.5\textwidth}
%  \centering
%  \includegraphics[scale=0.5]{outputs/q3/q3e-3-optimised-p}
%  \caption{EM optimised parameters: Trial 3}
%  \label{fig:3e-initialised-p-trial-3}
%\end{minipage}
%\end{figure}
